...
export default async function CourseDirectoryPage({
  ...
  const groups = Array.from(
    courses.reduce((map, course) => {
      ...
      return map;
    }, new Map<string, DirectoryGroup>()).values(),
  );
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-8 pb-24 sm:px-6 sm:py-12 md:pb-12">
      <header className="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
            Discover real university courses
          </h1>
          <p className="mt-3 max-w-2xl text-slate-600">
            Search public course pages without exposing private or unlisted courses.
          </p>
        </div>
        <Link
        ...
        >
          {studentId ? "Create course" : "Sign in to contribute"}
        </Link>
      </header>
      ...
      {studentId && primary ? (
        <nav aria-label="Directory scope" className="mt-7 grid grid-cols-2 rounded-xl bg-slate-100 p-1 sm:w-fit sm:min-w-80">
          <Link
            ...
            className={`min-h-11 rounded-lg px-4 py-3 text-center text-sm font-bold ${
              scope === "mine" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-600"
            }`}
          >
            My university
          </Link>
          <Link
            ...
            className={`min-h-11 rounded-lg px-4 py-3 text-center text-sm font-bold ${
              scope === "all" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-600"
            }`}
          >
            All universities
          </Link>
      ...
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        {scope === "all" ? (
          ...
          >
            <CourseOrganizationFilter defaultOrganization={selectedOrganization} />
          </Suspense>
        ...
        <form action="/courses" className="mt-5 space-y-4">
          ...
          {scope === "mine" && primary ? (
            <input type="hidden" name="organization" value={primary.organizationId} />
          ) : requestedOrganization ? (
            <input type="hidden" name="organization" value={requestedOrganization} />
          ) : null}
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button type="submit" className="min-h-11 self-end rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white">
              Search
            </button>
          </div>
          <details open={hasAdvancedFilters} className="rounded-xl border border-slate-200">
            ...
            <div className="grid gap-3 border-t border-slate-200 p-4 sm:grid-cols-2 lg:grid-cols-5">
              <FilterSelect label="Degree program" name="program" value={programSlug}>
              ...
              <label className="text-xs font-medium">
                Professor
                <input name="professor" defaultValue={professor} maxLength={120} className={`${filterClass} mt-1 w-full`} />
              </label>
              <FilterSelect label="Academic year" name="year" value={academicYear}>
                <option value="">All</option>
                {yearOptions.map(({ year }) => <option key={year} value={year}>{year}</option>)}
              </FilterSelect>
              <label className="text-xs font-medium">
                Semester
                <input name="semester" type="number" min={1} max={12} defaultValue={semester ?? ""} className={`${filterClass} mt-1 w-full`} />
              </label>
              <FilterSelect label="Canonical template" name="template" value={templateId}>
                <option value="">All</option>
      ...
      <div className="mt-6 flex items-center justify-between gap-3">
        <h2 className="text-sm font-semibold text-slate-500">
          {groups.length} {groups.length === 1 ? "university group" : "university groups"}
          {page > 1 ? ` on page ${page}` : ""}
        </h2>
        {search || requestedOrganization || hasAdvancedFilters ? (
          <Link href={clearHref} className="text-sm font-bold text-indigo-700">Clear filters</Link>
        ) : null}
      </div>
      ...
      {groups.length ? (
        <div className="mt-4 space-y-5">
          {groups.map((group) => (
            <details key={group.id} open className="group rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    {group.domain || group.website ? (
                      <p className="mt-1 truncate text-xs text-slate-500">{group.domain ?? group.website}</p>
                    ) : null}
                  </div>
                  <span className="shrink-0 rounded-full bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">
                    {group.publicCourseCount} public {group.publicCourseCount === 1 ? "course" : "courses"}
                  </span>
                </div>
              </summary>
              <ul className="grid gap-3 border-t border-slate-200 p-4 sm:grid-cols-2 lg:grid-cols-3">
                {group.courses.map((course) => (
                  <li key={course.slug}>
                    <Link href={`/courses/${course.slug}`} className="block h-full rounded-xl border border-slate-200 p-4 transition hover:border-indigo-300 hover:bg-indigo-50/30">
                      <div className="flex items-start justify-between gap-3">
                        <h3 className="font-bold">{course.localName}</h3>
                        {course.courseCode ? <span className="text-xs text-slate-500">{course.courseCode}</span> : null}
                      </div>
        ...
        <section className="mt-4 rounded-2xl border border-dashed border-slate-300 p-10 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">Try broader filters or create the first accurate page for this university.</p>
        </section>
      )}

      {page > 1 || hasNextPage ? (
        <nav aria-label="Course directory pages" className="mt-8 flex items-center justify-between gap-3">
          {page > 1 ? <Link href={pageHref(page - 1)} rel="prev" className={pageLinkClass}>Previous</Link> : <span />}
          <span className="text-sm text-slate-500">Page {page}</span>
          {hasNextPage ? <Link href={pageHref(page + 1)} rel="next" className={pageLinkClass}>Next</Link> : <span />}
        </nav>
      ) : null}
    </main>
  );
...
function FilterSelect({
  ...
  return (
    <label className="text-xs font-medium">
      {label}
      <select name={name} defaultValue={value} className={`${filterClass} mt-1 w-full`}>
        {children}
      </select>
    </label>
  );
